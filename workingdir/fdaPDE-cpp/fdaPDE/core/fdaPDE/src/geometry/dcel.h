   ...
   public:
    ...
    struct node_t {
        private:
         ...
         void set_coords(const coords_t& coords) {
             fdapde_assert((coords.rows() == 1 && coords.cols() == embed_dim) || (coords.rows() == embed_dim && coords.cols() == 1));
             if (coords.rows() == 1) {
                 coords_ = coords.transpose();
             } else {
                 coords_ = coords;
    ...
    halfedge_t* add_polygon(halfedge_t* v, const std::vector<node_t*>& nodes, bool building_dcel=false){
        ...
        //std::cout << "v: " << v->node()->coords() << std::endl;
        //std::cout << "v_cell" << c->id() << std::endl;
        //std::cout << "v_twin: " << v->twin()->node()->coords() << std::endl;
        //if(v->twin()->cell()) std::cout << "v_twin_cell: " << v->twin()->cell()->id() << std::endl;
        //std::cout << "v_next: " << v->next()->node()->coords() << std::endl;
        //std::cout << "v_next_cell: " << v->next()->cell()->id() << std::endl;
        //std::cout << "v_next_twin: " << v->next()->twin()->node()->coords() << std::endl;
        //if(v->next()->twin()->cell()) std::cout << "v_next_twin_cell: " << v->next()->twin()->cell()->id() << std::endl;
        ...
        for (int i = 0; i < nodes_polygon; ++i) {
            ...
            //std::cout << "h: " << h->node()->coords() << std::endl;
            //std::cout << "h_twin: " << h->twin()->node()->coords() << std::endl;
            //std::cout << "h_cell: " << h->cell()->id() << std::endl;
            //if(h->twin()->cell()) std::cout << "h_twin_cell: " << h->twin()->cell()->id() << std::endl;
        }
        // add edges
