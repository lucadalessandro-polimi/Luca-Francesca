...
template <int EmbedDim, typename DataType, typename SplitStrategy = internals::rtree_star_split>
class RTree {
   private:
    static constexpr int embed_dim = EmbedDim;
    ...
    template <typename SpatialObject_> struct is_valid_spatial_object {
        static constexpr bool value = requires(SpatialObject_ obj) {
            SpatialObject_::embed_dim;
            { obj.bbox() } -> std::convertible_to<std::array<double, 2 * embed_dim>>;
        } || requires(SpatialObject_ obj) {
            std::remove_pointer_t<std::remove_cvref_t<SpatialObject_>>::embed_dim;
            { obj->bbox() } -> std::convertible_to<std::array<double, 2 * embed_dim>>;
        };
    };
    template <typename SpatialObject_>
    ...
    struct bbox_t {
        ...
        explicit bbox_t(const SpatialObject_& obj): bbox_t([&]() {
                if constexpr (std::is_pointer_v<SpatialObject_>)
                    return obj->bbox();
                else
                    return obj.bbox();
            }()) { }
    ...
    struct item_t {
       private:
        ...
        union {
            node_t* node_;   // internal nodes: pointer to child
            DataType data_;       // leaf nodes: database index at which this spatial object is stored
        };
        bbox_t bbox_;   // smallest rectangle that spatially contains this item
       public:
        // constructor
        ...
        template <typename BBoxT>
            requires(std::is_convertible_v<BBoxT, bbox_t>)
        explicit item_t(DataType index, const BBoxT& bbox) : data_(index), type_(type_t::DATA), bbox_(bbox) { }
        explicit item_t(node_t* node) : node_(node), type_(type_t::NODE), bbox_(node_->bbox()) { }
        // copy/move semantic
        item_t(const item_t& other) = default;
        item_t(item_t&& other) = default;
    ...
    // structural tree node
    struct node_t {
        using bbox_t = typename RTree<EmbedDim, DataType, SplitStrategy>::bbox_t;
        using item_t = typename RTree<EmbedDim, DataType, SplitStrategy>::item_t;
        static constexpr int embed_dim = EmbedDim;
        ...
        while (true) {
            ...
            if (n1->size() > M_) {
                if (!n1->is_root() && !ctx[n1->level()]) {
                    ...
                    n1->recompute_bbox();
                    //parent->update_bbox_of(item_t(n1));
                    parent->update_bbox_of(*std::find_if(parent->begin(), parent->end(),[&](const item_t& it){ return it.node() == n1; }));
                    break;   // reinsertion stop propagation
                } else {
                    const auto& [e1, e2] = split_.apply(n1, n1->level());
                    n1 = e1;
            ...
            if (parent->size() < M_ + 1) { 
                ...
                for (auto& item : *parent) {
                    if (item.is_node() && item.node() != nullptr) {
                        item.bbox() = item.node()->bbox();
                    }
                }
            }
    ...
    // find leaf node containing obj. returns nullptr if no obj is found
    std::pair<node_t*, const item_t*> find_leaf_(const bbox_t& obj_bbox, DataType index) {
        node_t* curr;
        std::stack<node_t*> stack_;
        stack_.push(root_);
        while (!stack_.empty()) {
    ...
    // Propagate node elimination upward, adjusting covering rectangles as necessary
    void condense_tree_(node_t* l) {
        std::vector<std::pair<DataType, bbox_t>> Q;   // set of eliminated items
        std::vector<node_t*> killed;
        node_t* n = l;
        while (!n->is_root()) {
            node_t* parent = n->parent();
            if (n->size() < m_) {   // too few entries, eliminate node
                if (n->is_leaf()) {
                    for (const item_t& item : *n) { Q.emplace_back(item.data(), item.bbox()); }
                    if (std::find(killed.begin(), killed.end(), n) == killed.end())
                    killed.push_back(n);
                } else {
                    // if node was not a leaf, eliminate its entire subtree
                    std::stack<node_t*> stack_;
                ...
                //print_node_(n, n->level(), std::cout);
                //print_tree();
                std::cout << "Updating bbox for node " << n << std::endl;
                parent->update_bbox_of(item_t(n));
            }
            parent->recompute_bbox();
            n = parent;
        ...
        // free memory and insert
        for (node_t* node : killed) {
            std::cout << node << std::endl;
            print_node_(node, node->level(), std::cout);
            delete node;
        }
        for (const auto& [obj_data, obj_bbox] : Q) { insert(obj_data, obj_data); }
        return;
    }
    ...
    // pack data in (data.size() + M_ - 1) / M_ leaves
    std::vector<node_t*> str_make_leaves_(const std::vector<std::pair<DataType, bbox_t>>& data) {
        using point_t = std::array<double, embed_dim>;
        int n = data.size();
        std::vector<int> idxs(n);
        std::iota(idxs.begin(), idxs.end(), 0);
   ...
   public:
    ...
    template <typename SpatialObject_>
        requires(is_valid_spatial_object_v<SpatialObject_>)
    void insert(const SpatialObject_& obj, DataType index) {
        if constexpr (std::is_pointer_v<SpatialObject_>)
            fdapde_static_assert(std::remove_pointer_t<SpatialObject_>::embed_dim == embed_dim,INCORRECT_SPATIAL_OBJECT_EMBEDDING_DIMENSION);
        else
            fdapde_static_assert(SpatialObject_::embed_dim == embed_dim,INCORRECT_SPATIAL_OBJECT_EMBEDDING_DIMENSION);
        bbox_t obj_bbox = [&]() {
        if constexpr (std::is_pointer_v<SpatialObject_>)
            return bbox_t(obj->bbox());   
        else
            return bbox_t(obj.bbox());    
        }();
        //std::cout << "Inserting bbox 2: ["<< obj_bbox[0] << ", " << obj_bbox[1]<< ", " << obj_bbox[2] << ", " << obj_bbox[3] << "]"<< std::endl;
        // select a leaf where insert obj
        node_t* leaf_node = choose_leaf_(obj_bbox);
        leaf_node->insert(item_t(index, obj_bbox));
        // propagate tree rebalancing up
    ...
    template <typename SpatialObject_>
        requires(is_valid_spatial_object_v<SpatialObject_>)
    void erase(const SpatialObject_& obj, DataType index) {
        // find leaf containing obj
        bbox_t obj_box;
        if constexpr (std::is_pointer_v<SpatialObject_>)
            obj_box = bbox_t(obj->bbox());
        else
            obj_box = bbox_t(obj.bbox());
        const auto& [leaf_node, item] = find_leaf_(obj_box, index);
        if (!leaf_node) return;
        //std::cout << "removing bbox: ["<< obj_box[0] << ", " << obj_box[1]<< ", " << obj_box[2] << ", " << obj_box[3] << "]"<< std::endl;
        leaf_node->erase(*item);
        condense_tree_(leaf_node);
        //print_node_(leaf_node, leaf_node->level(), std::cout);
        // if the root node has only one child after the condensation, make the child the new root
        if (!root_->is_leaf() && root_->size() == 1) {
            node_t* new_root = root_->item(0).node();
            new_root->set_parent(nullptr);
    ...
    void bulk_load(const SpatialIndex& data) {
        ...
        int n = data.size();
        // compute bounding boxes of data
        std::vector<std::pair<DataType, bbox_t>> data_;
        data_.reserve(n);
        for (int i = 0; i < n; ++i) {
            if constexpr (std::is_pointer_v<std::decay_t<decltype(data[i])>>){
                data_.emplace_back(data[i], data[i]->bbox());
            }
            else
                data_.emplace_back(&data[i], data[i].bbox());
        }
    ...
    template <typename QueryObject>
        requires(std::is_convertible_v<QueryObject, std::array<double, 2 * embed_dim>>)
    std::vector<DataType> intersect_query(const QueryObject& query) const {
    ...
    template <typename PointT>
        requires(internals::is_vector_like_v<PointT>)
    std::vector<DataType> locate_query(PointT&& point) const {
        fdapde_assert(point.size() == embed_dim);
        std::vector<DataType> candidates;
        node_t* curr;
        std::stack<node_t*> stack_;
        ...
        while (!stack_.empty()) {
            curr = stack_.top();
            stack_.pop();
            // the bounding box doesn't contain point, skip entire subtree
            if (!curr->bbox().contains(point)) continue;

            const auto& coords = curr->bbox().coords();
            ...
            if (curr->is_leaf()) {
                for (const auto& item : *curr) {
                    if (item.bbox().contains(point)) { 
                        candidates.push_back(item.data()); 
                    }
                }
            } else {
                for (const auto& item : *curr) {
    ...
    void print_node_(const node_t* node, int depth, std::ostream& os) const {
        if (!node) return;

        std::string indent(depth * 2, ' ');
        const auto& b = node->bbox();

        os << indent << "• Node depth " << depth
        ...
        // --- stampa i contenuti (item_t) ---
        for (std::size_t i = 0; i < node->size(); ++i) {
            const auto& it = node->item(static_cast<int>(i));
            const auto& bb = it.bbox();
    ...
    void print_tree() const {
        dfs_visit_([](auto* node) {
            auto box = node->bbox();
            std::cout << std::string(2 * node->level(), ' ');
            std::cout << "• Node depth " << node->level()
                    << (node->is_leaf() ? " [leaf]" : "")
                    << " bbox: [" << box[0] << ", " << box[1]
                    << ", " << box[2] << ", " << box[3] << "]"
                    << " | size=" << node->size() << "\n";
            ...
            for (const auto& item : *node) {
                auto ib = item.bbox();
                std::cout << std::string(2 * node->level() + 2, ' ');
                std::cout << "↳ item " << i++
                        << " bbox: [" << ib[0] << ", " << ib[1]
                        << ", " << ib[2] << ", " << ib[3] << "]\n";
            }
