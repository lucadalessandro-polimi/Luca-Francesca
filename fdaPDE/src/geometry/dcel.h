   ...
   public:
    ...
    struct node_t {
        private:
         ...
         coords_t coords_;
         std::list<node_t>::iterator it_;   // iterator to the halfedge in the list
         ...
         std::list<node_t>::iterator it() const { return it_; }
    ...
    halfedge_t* remove_edge(halfedge_t* v1){        
        ...
        v1->node()->set_halfedge(v2->next());
        v2->node()->set_halfedge(v1->next());
    ...
    template <typename... Args> halfedge_t* emplace_halfedge(Args&&... args) {
        ...
        if(halfedges_.empty())
            ...
            id= std::prev(halfedges_end())->id() + 1; // get the last halfedge id and increment it
